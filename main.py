...
class QuantBetPipeline:
    ...
    def _run_value_betting(self, snapshots: List) -> Dict:
        ...
        snapshots_by_event = self._group_by_event(snapshots)
        all_value_bets = []
        # Probabilidades justas de ejemplo (en producción vendrían de un modelo)
    ...
    def _group_by_event(self, snapshots: List) -> Dict:
        grouped = {}
        for s in snapshots:
            grouped.setdefault(s.event_id, []).append(s)
        return grouped

    def list_events(self) -> List[str]:
        snapshots = self.provider.get_snapshots()
        return sorted(list({s.event_id for s in snapshots}))
